...
from .async_bridge import Qwen3Eagle3AsyncBridge
...
def maybe_create_qwen3_eagle3_bridge(
    ...
    draft_model_path = os.environ.get("VLLM_ONLINE_EAGLE3_DRAFT_MODEL") or getattr(
        draft_model_config,
        "model",
    )
    ...
    trainer = Qwen3Eagle3CpuTrainer(
        load_torch_eagle3_model(draft_model_path, dtype=_get_dtype()),
        trainer_config,
    )
    return Qwen3Eagle3AsyncBridge(
        ...
        update_interval=int(
            os.environ.get("VLLM_ONLINE_EAGLE3_UPDATE_INTERVAL", "1")
        ),
    )
...
def _get_dtype() -> torch.dtype:
    ...
    try:
        ...
        raise ValueError(
            "VLLM_ONLINE_EAGLE3_DTYPE must be one of "
            f"{sorted(_DTYPES)}, got {value!r}"
        ) from exc
